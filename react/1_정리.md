...
- CSR(Client-Side Rendering) : 클라이언트 측에서 렌더링을 수행
- SPA(Single-Page Apps) : 한 페이지에서 모든 동작을 처리하는 웹 앱
- SSG(Static-Site Generation) : 서버에서 모든 페이지를 미리 렌더링 한 후 클라이언트에 제공
- SSR(Server-Side Rendering) : 요청할 때마다 서버에서 HTML을 동적으로 렌더링하고 클라이언트에 전달
